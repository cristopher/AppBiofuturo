...
class LoginModel
{
    public static function login($user_email, $user_password, $set_remember_me_cookie = null)
    {
        ...
        if ($result->user_suspension_timestamp != null && $result->user_suspension_timestamp - time() > 0) {
            $suspensionTimer = Text::get('FEEDBACK_ACCOUNT_SUSPENDED') . round(abs($result->user_suspension_timestamp - time())/60/60, 2) . " horas";
            Session::add('feedback_negative', $suspensionTimer);
            return false;
        }

        if ($result->user_last_failed_login > 0) {
            self::resetFailedLoginCounterOfUser($result->user_id);
        }

        self::saveTimestampOfLoginOfUser($result->user_id);
    ...
    private static function validateAndGetUser($user_email, $user_password)
    {
        ...
        if (!$result) {
            ...
            Session::add('feedback_negative', Text::get('FEEDBACK_EMAIL_OR_PASSWORD_WRONG'));
            return false;
        }
        ...
        if (!password_verify($user_password, $result->user_password_hash)) {
            self::incrementFailedLoginCounterOfUser($result->user_id);
            Session::add('feedback_negative', Text::get('FEEDBACK_EMAIL_OR_PASSWORD_WRONG'));
            return false;
        }
    ...
    public static function incrementFailedLoginCounterOfUser($user_id)
    {
        ...
        $query->execute(array(':user_id' => $user_id, ':user_last_failed_login' => time() ));
    }

    public static function resetFailedLoginCounterOfUser($user_id)
    {
        $database = DatabaseFactory::getFactory()->getConnection();
    ...
    public static function saveTimestampOfLoginOfUser($user_id)
    {
        $database = DatabaseFactory::getFactory()->getConnection();
        ...
        $query = $database->prepare($sql);
        $query->execute(array(':user_id' => $user_id, ':user_last_login_timestamp' => time()));
    }
